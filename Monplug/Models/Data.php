...
class Data extends Database
{
    ...
    // public function addData($id, $code, $nom){

    //     $ajouter = $this->connect()->prepare('INSERT INTO communes (id, code, nom) VALUES (:id, :code, :nom)');

    //     $ajouter->bindParam (':id', $id); 
    ...
    public function allData(){
        ...
        $interventions = $this->connect()->prepare('SELECT * FROM communes WHERE id LIKE "DEP-%"');
    ...
    //     switch ($whichList) {
    //         case 'region':
    //             $departement = $this->connect()->prepare('SELECT id, nom FROM coviddatas WHERE code LIKE "REG-%"');
    //             break;
    ...
    public function communesList(){
        $communes = $this->connect()->prepare('SELECT code, nom FROM bubuu_communes WHERE code LIKE "DEP-%"');
        $communes ->execute();
        $dept = $communes->fetchAll(); //On lui demande de nous retourner dans la variable $int le résultat de notre requête sous forme de tableau.
        return $dept;
        var_dump($dept);
    }
